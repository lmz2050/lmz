...
import com.lmz.mike.auth.bean.LmzMenu;
import com.lmz.mike.auth.bean.LmzRole;
import com.lmz.mike.auth.utils.AccessTokenUtils;
...
public class AccessDecisionManagerIml implements AccessDecisionManager {
    ...
    public void decide(Authentication authentication, Object o, Collection<ConfigAttribute> collection) throws AccessDeniedException, InsufficientAuthenticationException {
        ...
        // URL 鉴权
        Iterator<LmzRole> iterator = accessTokenUtils.getRoleInfo().iterator();
        while (iterator.hasNext())
        {
            LmzRole baseRole = iterator.next();
            if (baseRole.getMenus().size() > 0 && checkSubModule(baseRole.getMenus())) {
                return;
            }
        }
    ...
    // 检查子模块权限
    private boolean checkSubModule(List<LmzMenu> modules) {

        Iterator<LmzMenu> iterator = modules.iterator();
        while (iterator.hasNext())
        {
            LmzMenu m = iterator.next();
            // 匹配当前应用的资源
            if(applicationName.equals(m.getSysCode())) {
                if (m.getIsOp()==1 && m.getActive() == 1 && m.getUrl()!= null && m.getUrl().trim().length()>0) {
                    if (matchUrl(url, m.getUrl()) && m.getHttpMethod().toUpperCase().contains(httpMethod.toUpperCase())) {
                        return true;
                    }
                }
                ...
                if (m.getSubMenus().size() > 0) {
                    if (checkSubModule(m.getSubMenus())) {
                        return true;
                    }
                }
            }
