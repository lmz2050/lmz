package cn.lmz.mike.web.util;

import cn.lmz.mike.common.M;
import cn.lmz.mike.common.web.init.LBeanInitCache;
import com.lmz.data.util.LmzU;
import com.lmz.data.util.invoke.Handler;
import com.lmz.data.util.invoke.InvokeBean;
...
public class BeanUtil {
  ...
  public static void handBean(Object bean,Handler hander){
    try {
    	O.pb("handBean - "+bean.getClass().getName()+" : ");
    	...
    	for(Map.Entry<String, PropertyDescriptor> entry:map.entrySet()){
    		...
    		if(mb!=null&&mb.getReadMethod()!=null){
    			...
    			O.pb("  - "+mb.getReadMethod().getName() +" ["+entry.getKey()+","+value+"]");
  ...
  public static Map<String,Object> beanToMap(Object bean,boolean withNullVal){
	   ...
	   Map<String,Object> bmap = new HashMap<String,Object>();
	    try {
	    	...
	    	Map<String,PropertyDescriptor> map = LBeanInitCache.getBeanInfo(bean.getClass());
	    	for(Map.Entry<String, PropertyDescriptor> entry:map.entrySet()){
	    		PropertyDescriptor mb= entry.getValue();
	    		if(mb!=null&&mb.getReadMethod()!=null){
	    			Object value = mb.getReadMethod().invoke(bean);
  ...
  public static Object setBean(Object bean,Map<String,Object> params){
	    try {
	    	...
	    	Map<String,PropertyDescriptor> map = LBeanInitCache.getBeanInfo(bean.getClass(),true);
	    	for(Map.Entry<String, Object> entry :params.entrySet()){
	    		...
	    		if(mb!=null&&mb.getWriteMethod()!=null){
	    			if(entry.getValue()!=null||(entry.getValue()==null&&!mb.getPropertyType().isPrimitive())){
	    				log.debug("  - "+mb.getWriteMethod().getName() +" ["+entry.getKey()+","+entry.getValue()+"]");
	    				if(mb.getPropertyType().equals(Integer.class)){
    						mb.getWriteMethod().invoke(bean, M.str.getInteger(entry.getValue()));
	    				}else{
		...
		while(it.hasNext()){
			...
			if(map.containsKey(en.getKey())){
    			...
    			if(val!=null||(val==null&&!mb.getPropertyType().isPrimitive())){
    				O.pb("  - "+mb.getWriteMethod().getName() +" ["+en.getKey()+","+val+"]"+mb.getPropertyType());
