...
public class XSSFExcel extends TimeLog implements IExcel{

	private static final Logger log = LoggerFactory.getLogger(XSSFExcel.class);
	...
	public void writeXlsWithRs(String xls,ResultSet rs)  throws Exception{
		...
		SXSSFWorkbook wb = new SXSSFWorkbook(5000);
		try {
			File xf = new File(xls);
			if(!xf.getParentFile().exists()){
			...
			FileOutputStream os = new FileOutputStream(xf);
			wb.write(os);
			os.close();
			wb.close();
			end("write end:"+xls+",count:"+count);
